...
public abstract class Board {
    ...
    */
    ...
    public void setWater(int col, int row){
        //Das Feld bekommt den Zustand Wasser
        board[col][row] = WATER;
    }
    ...
    public void changefieldCondition(int col, int row, int condition){
        board[col][row] = condition;
    }
    ...
    public void sunk(int col, int row, String shipType){
        //Das Feld wird in den Zustand versunken gesetzt
        //Jedes Schiff hat eine andere Zahl damit man spàter erkennt, welches Schiff versunken ist
        board[col][row] = 4;
    ...
    public void setRocket(int col, int row){
        //Für die Animation wird das Feld gelb gemacht
        board[col][row] = ROCKET;
    }
    ...
    public boolean isPossibleToSetShip(int col, int row, int lengthOfShip, int heightOfShip, String direction){
        ...
        */
        ...
        //Uberprufung ob das Schiff gesetzt werden kann
            ...
            if(col >= 0 &&  row >= 0 && col+lengthOfShip-1 <= 9 && row+heightOfShip-1<= 9) {
                for (int checkRow = 0; checkRow < heightOfShip; checkRow++) { 
                    for (int checkCol = 0; checkCol < lengthOfShip; checkCol++) {
                        if(getConditionOfField(col+checkCol, row+checkRow)==SHIP){
                            return false;
                        }
                    }
                }
