...
public class OwnBoard extends Board{
    ...
    // Das war fur einen Test
    
    /*public int getConditionOfFieldForBot(int col, int row){
        return probBoard[col][row];
    }
    
    public boolean isPossibleToSetShipBot(int col, int row, int lengthOfShip, int heightOfShip, String direction){
            if(col >= 0 &&  row >= 0 && col+lengthOfShip-1 <= 9 && row+heightOfShip-1<= 9) {
                for (int checkRow = 0; checkRow < heightOfShip; checkRow++) { 
                    for (int checkCol = 0; checkCol < lengthOfShip; checkCol++) {
                        if(getConditionOfFieldForBot(col+checkCol, row+checkRow)==1){
                            return false;
                        }
                    }
                }
    ...
    public void shootInProbBoard(int col, int row){
        int condition = probBoard[col][row];
        if(condition < WATERHIT){
            if(condition == WATER){
                probBoard[col][row] = WATERHIT;
                //System.out.println("No matches!");
            }
            else{
                System.out.println("Hit!");
                probBoard[col][row] = SHIPHIT;
            }
        } 
    }*/
